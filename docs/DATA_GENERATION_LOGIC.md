...
等级、标题、说明及四个解释因子。四类因子先得到 `39—78` 的实际原始分，
再线性归一化到 `45—95`，保证五个展示等级都可到达。当前规则版本为
`2026.07.2`；天秤座与双子座为 `91`，天秤座与巨蟹座为 `48`，切换选择会
真实改变结果。
...
遍历 78 组无序星座组合后，原始分的实际范围是 `39—78`。展示分使用线性
归一化：

```text
匹配分 = 45 + round((原始分 - 39) × 50 / 39)
```

这样最低组合对应 `45`、最高组合对应 `95`，五个等级都真实可达。归一化只是
产品刻度映射，不会增加新的占星依据；日志中的四个因子仍保留原始可解释分值。
...
归一化：
45 + round((75 - 39) × 50 / 39) = 91
```

结果为 `91`，等级为“高度共鸣”。旧规则额外加入固定基础分后直接截断到
`45—95`，实际只能得到 `49—88`，导致“高度共鸣”永远不可达；新规则移除
无解释意义的固定基础分，并将四个因子的实际范围映射到完整展示区间。
